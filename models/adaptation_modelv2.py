...
class CustomModel():
    ...
    def step(self, source_data, data_target_list, device):
        ...
        for i in range(self.num_target):
            ...
            ### Pseudo-label assignment:  entropy-guided pseudo-label
            ...
            for cl in range(class_num):
                ...
                mask2 = (predicted_entropy < np.maximum(0.2,thres[cl])).unsqueeze(dim=1)
                mask = mask1*mask2
                masks = masks | mask
            ...
            loss_kd = F.kl_div(student, teacher, reduction='none') + \
                      F.kl_div(teacher_copy, ensemble_teacher, reduction='none')
            mask = (teacher != 250).float()
            loss_kd = (loss_kd * mask).sum() / mask.sum()
            loss_kd /= self.num_target
            #### total loss
            loss += loss_CTS_all + loss_kd + self.opt.ratio * loss_CTS_transfered_all + 0.1*loss_ensemble_all
            loss.backward()
